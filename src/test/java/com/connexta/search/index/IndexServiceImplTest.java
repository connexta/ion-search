...
class IndexServiceImplTest {
              ...
              () ->
                  ((IndexService) new IndexServiceImpl(mockSearchManager, WebClient.create()))
                      .index(
                          datasetId,
                          storeMockWebServer
                              .url(String.format("/dataset/%s/file", datasetId))
                              .uri(),
                          storeMockWebServer
  ...
  //                    .index(datasetId, indexRequest.getFileLocation(),
  // indexRequest.getMetacardLocation(), irmUri));
  //    assertThat(thrown.getStatus(), is(HttpStatus.BAD_REQUEST));
  ...
  //              .setBody(
  //                  String.format(
  //                      "{ \"%s\" : \"All the color had been leached from Winterfell until only
  // grey and white remained.\" }",
  //                      EXT_EXTRACTED_TEXT))
  //              .setResponseCode(200));
  //      final URI irmUri = storeMockWebServer.url(String.format("/dataset/%s/irm",
  // datasetId)).uri();
  ...
  //    final String body =
  //        String.format(
  //            "{ \"%s\" : \"All the color had been leached from Winterfell until only grey and
  // white remained.\" }",
  //            EXT_EXTRACTED_TEXT);
  //
  //    final URI irmUri;
  ...
  //
  //      ((IndexService) new IndexServiceImpl(mockSearchManager, WebClient.create()))
  //          .index(datasetId, indexRequest.getFileLocation(), indexRequest.getMetacardLocation(),
  // irmUri);
  //    }
  //
  ...
  //    assertThat(IOUtils.toString(inputStreamCaptor.getValue(), StandardCharsets.UTF_8),
  // is(body));
  //  }
